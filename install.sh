#!/usr/bin/env bash
# Himalaya Code installer
#
# Detects the host OS, verifies the Rust toolchain (rustc + cargo),
# builds the `Himalaya` binary from the `rust/` workspace, and runs a
...
EOF
    printf '%s\n' "${COLOR_RESET}"
    printf '%sHimalaya Code installer%s\n' "${COLOR_DIM}" "${COLOR_RESET}"
}
...
${COLOR_DIM}---------------${COLOR_RESET}
  ...
  ${COLOR_BOLD}2. Linux: missing system packages${COLOR_RESET}
     The build needs git, pkg-config, and a C compiler (gcc) to compile
     bundled native dependencies (bzip2, zstd, oniguruma).
     Debian/Ubuntu:
       sudo apt-get update && sudo apt-get install -y \
         build-essential pkg-config git ca-certificates
     Fedora/RHEL:
       sudo dnf install -y git pkgconf-pkg-config gcc
     Arch:
       sudo pacman -S --needed git pkgconf base-devel
  ...
  ${COLOR_BOLD}5. Build fails partway through${COLOR_RESET}
     ...
     If the failure mentions a C compiler or native library, check step 2.

  ${COLOR_BOLD}6. 'Himalaya' not found after install${COLOR_RESET}
     The binary lives at:
       rust/target/${BUILD_PROFILE}/Himalaya
...
if [ "${OS_FAMILY}" = "linux" ]; then
    if require_cmd pkg-config; then
        ...
        warn "pkg-config not found — required by native C dependency build scripts"
        warn "Install with: sudo apt-get install pkg-config"
    fi
    if require_cmd gcc; then
        ok "gcc found: $(gcc --version | head -1)"
    else
        warn "gcc not found — required to compile native C dependencies (bzip2, zstd, oniguruma)"
        warn "Install with: sudo apt-get install build-essential"
    fi
fi
...
step "Building the Himalaya workspace (${BUILD_PROFILE})"
...
info "cargo build jobs: ${BUILD_JOBS}"
...
Himalaya_BIN="${RUST_DIR}/target/${BUILD_PROFILE}/Himalaya"

if [ ! -x "${Himalaya_BIN}" ]; then
    error "Expected binary not found at ${Himalaya_BIN}"
