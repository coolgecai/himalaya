# 🦞 Himalaya Code — Rust Implementation

A high-performance Rust rewrite of the Himalaya Code CLI agent harness. Built for speed, safety, and native tool execution.
...
cd rust/
cargo run -p rusty-Himalaya-cli -- --help

# Build the workspace
cargo build --workspace

# Run the interactive REPL
cargo run -p rusty-Himalaya-cli -- --model Himalaya-opus-4-6

# One-shot prompt
cargo run -p rusty-Himalaya-cli -- prompt "explain this codebase"

# JSON output for automation
cargo run -p rusty-Himalaya-cli -- --output-format json prompt "summarize src/main.rs"
```
...
```bash
cargo run -p rusty-Himalaya-cli -- login
```
...
- `crates/rusty-Himalaya-cli/tests/mock_parity_harness.rs` — clean-env CLI harness
...
| Todo tracking | ✅ |
| Notebook editing | ✅ |
| Himalaya.md / project memory | ✅ |
| Config file hierarchy (`.Himalaya.json` + merged config sections) | ✅ |
| Permission system | ✅ |
...
See [`../USAGE.md`](../USAGE.md) for usage examples and run `cargo run -p rusty-Himalaya-cli -- --help` for the live canonical command list.
...
└── crates/
    ...
    ├── rusty-Himalaya-cli/   # Main CLI binary (`Himalaya`)
...
- **Binary name:** `Himalaya`
- **Default model:** `Himalaya-opus-4-6`
