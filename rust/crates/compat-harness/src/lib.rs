...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    if let Some(explicit) = std::env::var_os("Himalaya_CODE_UPSTREAM") {
        candidates.push(PathBuf::from(explicit));
    }

    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("Himalaya-code"));
        candidates.push(ancestor.join("Himalayad-code"));
    }
    ...
    candidates.push(primary_repo_root.join("vendor").join("Himalaya-code"));
...
pub fn extract_bootstrap_plan(source: &str) -> BootstrapPlan {
    ...
    if source.contains("--Himalaya-in-chrome-mcp") {
