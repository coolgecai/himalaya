...
pub const DEFAULT_MODEL: &str = "Himalaya-sonnet-4-6";
    ...
    match scenario {
        ...
        Scenario::RepoReplayEval => {
            ...
            match (
                ...
                (Some((read_output, _)), Some((grep_output, _))) => final_text_sse(&format!(
                    "repo replay eval complete: {} / {} functions",
                    extract_read_content(read_output)
                        .lines()
                        .next()
                        .unwrap_or_default(),
                ...
                _ => tool_uses_sse(&[
                    ToolUseSse {
                        tool_id: "toolu_repo_replay_read",
                        tool_name: "read_file",
                        partial_json_chunks: &[r#"{"path":"src/lib.rs"}"#],
                    },
                    ToolUseSse {
                        tool_id: "toolu_repo_replay_grep",
                        tool_name: "grep_search",
                        partial_json_chunks: &[
                            "{\"pattern\":\"fn ",
                            "[a-z_]\",\"path\":\"src\"",
...
fn build_message_response(request: &MessageRequest, scenario: Scenario) -> MessageResponse {
    match scenario {
                ...
                _ => tool_message_response_many(
                    ...
                    &[
                        ...
                        ToolUseMessage {
                            tool_id: "toolu_repo_replay_grep",
                            tool_name: "grep_search",
                            input: json!({"pattern": "fn [a-z_]", "path": "src", "output_mode": "count"}),
                        },
                    ],
                ),
            }
        }
        Scenario::ReasoningStep => MessageResponse {
            id: "msg_reasoning_step".to_string(),
            kind: "message".to_string(),
            role: "assistant".to_string(),
            content: vec![
                OutputContentBlock::Thinking {
                    thinking: "check stream contract".to_string(),
                    signature: Some("sig_reasoning_contract".to_string()),
                },
                OutputContentBlock::Text {
                    text: "reasoning step complete.".to_string(),
                },
            ],
        ...
        Scenario::RedactedThinking => MessageResponse {
            id: "msg_redacted_thinking".to_string(),
            kind: "message".to_string(),
            role: "assistant".to_string(),
            content: vec![
                OutputContentBlock::RedactedThinking {
                    data: json!({"opaque":"redacted-contract"}),
                },
                OutputContentBlock::Text {
                    text: "redacted thinking complete.".to_string(),
                },
            ],
            model: DEFAULT_MODEL.to_string(),
            stop_reason: Some("end_turn".to_string()),
...
fn reasoning_step_sse() -> String {
    ...
    append_sse(
        ...
        json!({
            "type": "content_block_delta",
            "index": 0,
            "delta": {"type": "thinking_delta", "thinking": "check stream "}
        }),
    );
    append_sse(
        &mut body,
        ...
        json!({
            "type": "content_block_delta",
            "index": 0,
            "delta": {"type": "thinking_delta", "thinking": "contract"}
        }),
    );
    append_sse(
        &mut body,
        ...
        json!({
            "type": "content_block_delta",
            "index": 0,
            "delta": {"type": "signature_delta", "signature": "sig_reasoning_contract"}
        }),
    );
    append_sse(
        &mut body,
    ...
    append_sse(
        ...
        json!({
            "type": "content_block_delta",
            "index": 1,
            "delta": {"type": "text_delta", "text": "reasoning step complete."}
        }),
    );
    append_sse(
        &mut body,
...
fn redacted_thinking_sse() -> String {
    let mut body = String::new();
    append_sse(
        &mut body,
        "message_start",
    ...
    append_sse(
        ...
        json!({
            "type": "content_block_start",
            "index": 0,
            "content_block": {"type": "redacted_thinking", "data": {"opaque":"redacted-contract"}}
        }),
    );
    append_sse(
        &mut body,
    ...
    append_sse(
        ...
        json!({
            "type": "content_block_delta",
            "index": 1,
            "delta": {"type": "text_delta", "text": "redacted thinking complete."}
        }),
    );
    append_sse(
        &mut body,
