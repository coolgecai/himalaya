...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "read_file",
            description: "Read a file from the workspace. Supports plain text files as well as binary document formats: PDF, DOCX, XLSX, PPTX (text is extracted automatically). Use this tool for any file the user references, regardless of extension.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string" },
        ...
        ToolSpec {
            name: "Config",
            description: "Get or set Himalaya Code settings.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "setting": { "type": "string" },
...
fn execute_tool_with_enforcer(
    ...
    match name {
        ...
        "generate_file" => from_value::<GenerateFileInput>(input).and_then(run_generate_file),
...
fn verification_handoff_for_task(task: &runtime::task_registry::Task) -> serde_json::Value {
    let mut prompt = format!(
        "Verify task `{}` before it is reported complete.\n\nObjective:\n{}\n",
        task.task_id, task.prompt
    );

    if let Some(packet) = &task.task_packet {
        prompt.push_str("\nScope:\n");
        prompt.push_str(&packet.scope);
        prompt.push_str("\n\nAcceptance tests:\n");
        ...
        prompt.push_str("\nReporting contract:\n");
        prompt.push_str(&packet.reporting_contract);
        prompt.push('\n');
    } else {
        prompt.push_str("\nNo explicit acceptance tests were provided; inspect the work and run the most relevant checks before reporting pass or fail.\n");
    }
...
fn run_task_create(input: TaskCreateInput) -> Result<String, String> {
    ...
    to_pretty_json(json!({
        ...
        "progress_ledger": registry.ledger_for_task(&task.task_id),
        "verification_handoff": verification_handoff_for_task(&task)
    }))
}
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("Himalayad-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("HimalayaD_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("HimalayaD_TODO_STORE") {
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("Himalayad-agent-{}", job.manifest.agent_id);
    ...
    while let Some(event) = stream.next_event().await? {
        match event {
            ...
            ApiStreamEvent::ContentBlockStart(start) => {
                // Pre-register thinking blocks so deltas can accumulate
            ...
            ApiStreamEvent::ContentBlockDelta(delta) => match delta.delta {
                ...
                ContentBlockDelta::ThinkingDelta { thinking } => {
                    if let Some((text, _)) = pending_thinking.get_mut(&delta.index) {
            ...
            ApiStreamEvent::ContentBlockStop(stop) => {
                ...
                if let Some((thinking, signature)) = pending_thinking.remove(&stop.index) {
                    events.push(AssistantEvent::ReasoningStep(
                        runtime::ReasoningStep::Analysis {
                            content: thinking,
                            confidence: None,
                            signature,
                        },
...
impl ToolExecutor for SubagentToolExecutor {
    fn available_tools(&self) -> Vec<runtime::Tool> {
        tool_specs_for_allowed_tools(Some(&self.allowed_tools))
            .into_iter()
            .map(|spec| {
                tool_from_profile(spec.name, Some(spec.description), Some(&spec.input_schema))
            })
            .collect()
    }
...
fn push_output_block(
    ...
    match block {
        ...
        OutputContentBlock::RedactedThinking { data } => {
            events.push(AssistantEvent::ReasoningStep(
                runtime::ReasoningStep::RedactedThinking {
                    data: data.to_string(),
                },
            ));
        }
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("HimalayaD_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".Himalayad-agents"));
    }
    Ok(cwd.join(".Himalayad-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "Himalayaguide" | "Himalayaguideagent" | "guide" => String::from("Himalaya-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".Himalaya").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("Himalaya_CONFIG_HOME") {
        ...
        .map_err(|_| {
            String::from(
                "HOME is not set (on Windows, set USERPROFILE or HOME, \
                 or use Himalaya_CONFIG_HOME to point directly at the config directory)",
            )
        })?;
    Ok(PathBuf::from(home).join(".Himalaya"))
}
...
mod tests {
    ...
    fn exposes_mvp_tools() {
        ...
        assert!(!names.contains(&"TestingPermission"));
    }
    ...
    fn worker_tools_gate_prompt_delivery_until_ready_and_support_auto_trust() {
        ...
        assert_eq!(
            accepted_output["events"]
                .as_array()
                .expect("events")
                .last()
                .expect("prompt accepted event")["kind"],
            "prompt_accepted"
        );
    }
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .Himalaya/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let Himalaya_dir = worktree.join(".Himalaya");
        fs::create_dir_all(&Himalaya_dir).expect("create .Himalaya dir");
        ...
        fs::write(Himalaya_dir.join("settings.json"), settings).expect("write settings");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .Himalaya/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".Himalaya").join("worker-state.json");
    ...
    fn stall_detect_and_resolve_trust_end_to_end() {
        ...
        // 3. Himalayahip calls WorkerResolveTrust to unblock
    ...
    fn stall_detect_and_restart_recovery_end_to_end() {
        // Worker stalls at trust_required, Himalayahip restarts instead of resolving
        let created = execute_tool(
            "WorkerCreate",
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        // sets HimalayaD_WEB_SEARCH_BASE_URL. Without the lock, parallel test
        ...
        std::env::remove_var("HimalayaD_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("HimalayaD_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("HimalayaD_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("HimalayaD_TODO_STORE");
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".Himalaya").join("skills").join("plan");
        let command_dir = root.join(".Himalaya").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".Himalaya/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".Himalaya/commands/handoff.md"));
    ...
    #[test]
    fn skill_loads_project_local_Himalaya_skill_prompt() {
        let _guard = env_lock().lock().expect("env lock should acquire");
        let root = temp_path("project-skills");
        let home = root.join("home");
        let workspace = root.join("workspace");
        let nested = workspace.join("nested");
        let skill_dir = workspace.join(".Himalaya").join("skills").join("trace");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(output["path"]
            .as_str()
            .expect("path")
            .ends_with(".Himalaya/skills/trace/SKILL.md"));
    ...
    #[test]
    fn skill_loads_learned_skill_from_Himalaya_config_dir() {
        let _guard = env_lock().lock().expect("env lock should acquire");
        let root = temp_path("Himalaya-config-learned-skill");
        let home = root.join("home");
        let Himalaya_config_dir = root.join("Himalaya-config");
        let learned_skill_dir = Himalaya_config_dir
            .join("skills")
            .join("omc-learned")
            .join("learned");
        ...
        let original_config_home = std::env::var("Himalaya_CONFIG_HOME").ok();
        let original_codex_home = std::env::var("CODEX_HOME").ok();
        let original_Himalaya_config_dir = std::env::var("Himalaya_CONFIG_DIR").ok();
        std::env::set_var("HOME", &home);
        std::env::remove_var("Himalaya_CONFIG_HOME");
        std::env::remove_var("CODEX_HOME");
        std::env::set_var("Himalaya_CONFIG_DIR", &Himalaya_config_dir);
        ...
        match original_Himalaya_config_dir {
            Some(value) => std::env::set_var("Himalaya_CONFIG_DIR", value),
            None => std::env::remove_var("Himalaya_CONFIG_DIR"),
        }
        fs::remove_dir_all(root).expect("temp tree should clean up");
    }

    #[test]
    fn skill_loads_direct_skill_and_legacy_command_from_Himalaya_config_dir() {
        let _guard = env_lock().lock().expect("env lock should acquire");
        let root = temp_path("Himalaya-config-direct-skill");
        let home = root.join("home");
        let Himalaya_config_dir = root.join("Himalaya-config");
        let skill_dir = Himalaya_config_dir.join("skills").join("statusline");
        let command_dir = Himalaya_config_dir.join("commands");
        fs::create_dir_all(&skill_dir).expect("direct skill dir should exist");
        fs::create_dir_all(&command_dir).expect("command dir should exist");
        fs::write(
            skill_dir.join("SKILL.md"),
            "---\nname: statusline\ndescription: Himalaya config skill\n---\n# statusline\n",
        )
        .expect("direct skill file should exist");
        fs::write(
            command_dir.join("doctor-check.md"),
            "---\nname: doctor-check\ndescription: Himalaya config command\n---\n# doctor-check\n",
        )
        ...
        assert_eq!(direct_skill_output["description"], "Himalaya config skill");
    ...
    fn skill_loads_project_local_legacy_command_markdown() {
        ...
        let workspace = root.join("workspace");
        let nested = workspace.join("nested");
        let command_dir = workspace.join(".Himalaya").join("commands");
        ...
        assert!(output["path"]
            .as_str()
            .expect("path")
            .ends_with(".Himalaya/commands/team.md"));
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("HimalayaD_AGENT_STORE", &dir);
        ...
        std::env::remove_var("HimalayaD_AGENT_STORE");
    ...
    fn agent_fake_runner_can_persist_completion_and_failure() {
        ...
        let completed = execute_agent_with_spawn(
            AgentInput {
                ...
                model: Some("Himalaya-sonnet-4-6".to_string()),
            },
            |job| {
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".Himalaya")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".Himalaya")).expect("cwd dir");
        std::fs::write(
            home.join(".Himalaya").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        let local_settings =
            std::fs::read_to_string(cwd.join(".Himalaya").join("settings.local.json"))
...
"#,
    ...
    #[test]
    fn read_only_enforcer_denies_all_high_risk_tools_before_dispatch() {
        let registry = read_only_registry();
        for (tool_name, input) in [
            ("Agent", json!({ "prompt": "inspect" })),
            ("REPL", json!({ "code": "println!()" })),
            ("PowerShell", json!({ "command": "Write-Output hi" })),
            ("WorkerCreate", json!({ "cwd": "/tmp" })),
            ("RemoteTrigger", json!({ "url": "https://example.invalid" })),
            ("MCP", json!({ "server": "demo", "tool": "echo" })),
        ] {
            let err = registry
                .execute(tool_name, &input)
                .expect_err(&format!("{tool_name} should be denied before dispatch"));
            assert!(
                err.contains("current mode is read-only"),
                "{tool_name} should cite read-only denial before validation or execution: {err}"
            );
        }
    }

    #[test]
    fn read_only_enforcer_denies_plugin_write_tool_before_dispatch() {
        let plugin_tool = PluginTool::new(
            "plugin-id",
            "Plugin",
            PluginToolDefinition {
                name: "plugin_write".to_string(),
                description: Some("Write through a plugin".to_string()),
                input_schema: json!({
                    "type": "object",
            ...
            "definitely-not-a-real-himalaya-plugin-command",
            Vec::new(),
            PluginToolPermission::WorkspaceWrite,
            None,
        );
        let mut registry = GlobalToolRegistry::with_plugin_tools(vec![plugin_tool])
            .expect("plugin tool should register");
        let policy = registry.permission_specs(None).unwrap().into_iter().fold(
        ...
        let err = registry
            .execute("plugin_write", &json!({ "path": "generated.txt" }))
            .expect_err("plugin write tool should be denied before command execution");

        assert!(
            err.contains("current mode is read-only"),
            "plugin tool should cite read-only denial before dispatch: {err}"
        );
    }
    ...
    fn provider_runtime_client_chain_uses_only_primary_when_no_fallbacks_configured() {
        ...
        assert_eq!(client.chain[0].model, "Himalaya-sonnet-4-6");
    ...
    fn provider_runtime_client_chain_primary_override_replaces_constructor_model() {
        ...
        let fallback_config = ProviderFallbackConfig::new(
            ...
            vec!["Himalaya-sonnet-4-6".to_string()],
        );

        // when
        let client = ProviderRuntimeClient::new_with_fallback_config(
            "Himalaya-haiku-4-5-20251213".to_string(),
            BTreeSet::new(),
            &fallback_config,
        )
        ...
        assert_eq!(client.chain[1].model, "Himalaya-sonnet-4-6");
    ...
    fn provider_runtime_client_chain_skips_fallbacks_missing_credentials() {
        ...
        assert_eq!(client.chain.len(), 2);
        assert_eq!(client.chain[0].model, "Himalaya-sonnet-4-6");
        assert_eq!(client.chain[1].model, "Himalaya-haiku-4-5-20251213");
    ...
    fn run_task_packet_creates_packet_backed_task() {
        ...
        assert_eq!(output["task_packet"]["repo"], "Himalaya-code-parity");
        assert_eq!(output["verification_handoff"]["tool"], "Agent");
        assert_eq!(
            output["verification_handoff"]["subagent_type"],
            "Verification"
        );
        assert!(output["verification_handoff"]["prompt"]
            .as_str()
            .expect("verification prompt")
            .contains("cargo test --workspace"));
        assert_eq!(output["progress_ledger"][0]["event"], "created");
    }
