...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "config",
        aliases: &[],
        summary: "Inspect Himalaya config files or merged sections",
    ...
    SlashCommandSpec {
        name: "memory",
        aliases: &[],
        summary: "Inspect loaded Himalaya instruction memory files",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
        name: "init",
        aliases: &[],
        summary: "Create a starter Himalaya.md for this repo",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
    ...
    SlashCommandSpec {
        ...
        summary: "Manage Himalaya Code plugins",
        argument_hint: Some(
    ...
    SlashCommandSpec {
        name: "plan",
        aliases: &[],
        summary: "Generate a local task plan",
        argument_hint: Some("[task]"),
        resume_supported: true,
    },
    SlashCommandSpec {
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectHimalaya,
            ancestor.join(".Himalaya").join(leaf),
        );
        push_unique_root(
            &mut roots,
    ...
    if let Ok(Himalaya_config_home) = env::var("Himalaya_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserHimalayaConfigHome,
            PathBuf::from(Himalaya_config_home).join(leaf),
        );
    }
    ...
    if let Ok(Himalaya_config_dir) = env::var("Himalaya_CONFIG_DIR") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserHimalaya,
            PathBuf::from(Himalaya_config_dir).join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectHimalaya,
            ancestor.join(".Himalaya").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectHimalaya,
            ancestor.join(".Himalaya").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Ok(Himalaya_config_home) = env::var("Himalaya_CONFIG_HOME") {
        let Himalaya_config_home = PathBuf::from(Himalaya_config_home);
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserHimalayaConfigHome,
            Himalaya_config_home.join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserHimalayaConfigHome,
            Himalaya_config_home.join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserHimalaya,
            home.join(".Himalaya").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserHimalaya,
            home.join(".Himalaya").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Ok(Himalaya_config_dir) = env::var("Himalaya_CONFIG_DIR") {
        let Himalaya_config_dir = PathBuf::from(Himalaya_config_dir);
        let skills_dir = Himalaya_config_dir.join("skills");
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserHimalaya,
            skills_dir.clone(),
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    if let Ok(Himalaya_config_home) = env::var("Himalaya_CONFIG_HOME") {
        return Ok(PathBuf::from(Himalaya_config_home).join("skills"));
    }
    if let Ok(codex_home) = env::var("CODEX_HOME") {
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       Himalaya agents".to_string(),
        "  Sources          .Himalaya/agents, ~/.Himalaya/agents, $Himalaya_CONFIG_HOME/agents"
            .to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "Himalaya agents [list|help]",
            "sources": [".Himalaya/agents", "~/.Himalaya/agents", "$Himalaya_CONFIG_HOME/agents"],
        },
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       Himalaya mcp [list|show <server>|help]".to_string(),
        "  Sources          .Himalaya/settings.json, .Himalaya/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "Himalaya mcp [list|show <server>|help]",
            "sources": [".Himalaya/settings.json", ".Himalaya/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
fn definition_source_id(source: DefinitionSource) -> &'static str {
    match source {
        DefinitionSource::ProjectHimalaya | DefinitionSource::ProjectCodex => "project_Himalaya",
        DefinitionSource::UserHimalayaConfigHome | DefinitionSource::UserCodexHome => {
...
mod tests {
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/model Himalaya-opus"),
            Ok(Some(SlashCommand::Model {
                model: Some("Himalaya-opus".to_string()),
            }))
        );
        assert_eq!(
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Himalaya Code plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn ignores_unknown_or_runtime_bound_slash_commands() {
        ...
        assert!(
            handle_slash_command("/model Himalaya", &session, CompactionConfig::default())
                .is_none()
        );
    ...
    fn lists_agents_from_project_and_user_roots() {
        ...
        let user_agents = user_home.join(".Himalaya").join("agents");
    ...
    fn renders_agents_reports_as_json() {
        ...
        assert_eq!(report["agents"][2]["shadowed_by"]["id"], "project_Himalaya");

        let help = handle_agents_slash_command_json(Some("help"), &workspace).expect("agents help");
        assert_eq!(help["kind"], "agents");
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["direct_cli"], "Himalaya agents [list|help]");
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".Himalaya").join("commands");
        ...
        let roots = vec![
            ...
            SkillRoot {
                source: DefinitionSource::ProjectHimalaya,
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let legacy_commands = workspace.join(".Himalaya").join("commands");
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       Himalaya agents"));
    ...
    fn discovers_omc_skills_from_project_and_user_compatibility_roots() {
        ...
        let Himalaya_config_dir = temp_dir("skills-omc-Himalaya-config");
        ...
        let Himalaya_config_skills = Himalaya_config_dir.join("skills");
        let Himalaya_config_commands = Himalaya_config_dir.join("commands");
        let learned_skills = Himalaya_config_dir.join("skills").join("omc-learned");
        let original_home = std::env::var_os("HOME");
        let original_Himalaya_config_dir = std::env::var_os("Himalaya_CONFIG_DIR");
        ...
        write_skill(
            &Himalaya_config_skills,
            "statusline",
            "Himalaya config skill guidance",
        );
        write_legacy_command(
            &Himalaya_config_commands,
            "doctor-check",
            "Himalaya config command guidance",
        );
        ...
        assert!(report.contains("statusline · Himalaya config skill guidance"));
        assert!(
            report.contains("doctor-check · Himalaya config command guidance · legacy /commands")
        );
        ...
        restore_env_var("Himalaya_CONFIG_DIR", original_Himalaya_config_dir);
        let _ = fs::remove_dir_all(workspace);
        let _ = fs::remove_dir_all(user_home);
        let _ = fs::remove_dir_all(Himalaya_config_dir);
    }
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       Himalaya mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".Himalaya")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".Himalaya").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".Himalaya").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".Himalaya/settings.json");
