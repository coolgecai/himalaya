...
const MANIFEST_RELATIVE_PATH: &str = ".Himalaya-plugin/plugin.json";
const BUNDLED_PLUGINS_ENV_VAR: &str = "HIMALAYA_BUNDLED_PLUGINS_DIR";
const INSTALLED_BUNDLED_PLUGINS_RELATIVE_PATH: &str = "share/himalaya-code/bundled";
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("HimalayaD_PLUGIN_ID", &self.plugin_id)
            .env("HimalayaD_PLUGIN_NAME", &self.plugin_name)
            .env("HimalayaD_TOOL_NAME", &self.definition.name)
            .env("HimalayaD_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("HimalayaD_PLUGIN_ROOT", root.display().to_string());
        }
...
impl PluginManager {
    ...
    pub fn bundled_root() -> PathBuf {
        if let Some(path) = std::env::var_os(BUNDLED_PLUGINS_ENV_VAR) {
            return PathBuf::from(path);
        }

        if let Some(path) = installed_bundled_root_from_executable() {
            if path.exists() {
                return path;
            }
        }

        let source_root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("bundled");
        if source_root.exists() {
            return source_root;
        }

        PathBuf::from("/usr/share/himalaya-code/bundled")
    }
...
fn load_manifest_from_path(
    ...
    let compatibility_errors = detect_himalaya_code_manifest_contract_gaps(&raw_json);
...
fn detect_himalaya_code_manifest_contract_gaps(
    raw_manifest: &Value,
    ...
    for (field, detail) in [
        (
            "skills",
            "plugin manifest field `skills` uses the Himalaya Code plugin contract; `Himalaya` does not load plugin-managed skills and instead discovers skills from local roots such as `.Himalaya/skills`, `.omc/skills`, `.agents/skills`, `~/.omc/skills`, and `~/.Himalaya/skills/omc-learned`.",
        ),
        (
            "mcpServers",
            "plugin manifest field `mcpServers` uses the Himalaya Code plugin contract; `Himalaya` does not import MCP servers from plugin manifests.",
        ),
        (
            "agents",
            "plugin manifest field `agents` uses the Himalaya Code plugin contract; `Himalaya` does not load plugin-managed agent markdown catalogs from plugin manifests.",
        ),
    ] {
    ...
    {
        errors.push(PluginManifestValidationError::UnsupportedManifestContract {
            detail: "plugin manifest field `commands` uses Himalaya Code-style directory globs; `Himalaya` slash dispatch is still built-in and does not load plugin slash command markdown files.".to_string(),
        });
    }
    ...
    if let Some(hooks) = root.get("hooks").and_then(Value::as_object) {
        for hook_name in hooks.keys() {
            if !matches!(
                ...
                errors.push(PluginManifestValidationError::UnsupportedManifestContract {
                    detail: format!(
                        "plugin hook `{hook_name}` uses the Himalaya Code lifecycle contract; `Himalaya` plugins currently support only PreToolUse, PostToolUse, and PostToolUseFailure."
                    ),
                });
            }
        }
...
fn installed_bundled_root_from_executable() -> Option<PathBuf> {
    let executable = std::env::current_exe().ok()?;
    let binary_dir = executable.parent()?;
    let prefix_dir = binary_dir.parent()?;
    Some(prefix_dir.join(INSTALLED_BUNDLED_PLUGINS_RELATIVE_PATH))
}
...
mod tests {
  ...
  "commands": [
    ...
    fn write_tool_plugin_with_name(root: &Path, name: &str, version: &str, tool_name: &str) {
        ...
        write_file(
            &script_path,
            "#!/bin/sh\nINPUT=$(cat)\nprintf '{\"plugin\":\"%s\",\"tool\":\"%s\",\"input\":%s}\\n' \"$HimalayaD_PLUGIN_ID\" \"$HimalayaD_TOOL_NAME\" \"$INPUT\"\n",
        );
        #[cfg(unix)]
        {
  ...
  "commands": [
    ...
    #[test]
    fn load_plugin_from_directory_rejects_himalaya_code_manifest_contracts_with_guidance() {
        let root = temp_dir("manifest-Himalaya-code-contract");
        write_file(
            root.join(MANIFEST_FILE_NAME).as_path(),
            r#"{
  "name": "oh-my-Himalayacode",
  "version": "4.10.2",
  "description": "Himalaya Code plugin manifest",
  "hooks": {
  ...
  "mcpServers": "./.mcp.json"
        ...
        let error = load_plugin_from_directory(&root)
            .expect_err("Himalaya Code plugin manifest should fail with guidance");
        let rendered = error.to_string();
        assert!(rendered.contains("field `skills` uses the Himalaya Code plugin contract"));
        assert!(rendered.contains("field `mcpServers` uses the Himalaya Code plugin contract"));
        assert!(rendered.contains("field `agents` uses the Himalaya Code plugin contract"));
        assert!(rendered.contains("field `commands` uses Himalaya Code-style directory globs"));
        assert!(rendered.contains("hook `SessionStart` uses the Himalaya Code lifecycle contract"));
