...
mod tests {
    ...
    fn json_deserialize_error_includes_provider_model_and_truncated_body_snippet() {
        ...
        let error = ApiError::json_deserialize("Anthropic", "Himalaya-opus-4-6", &raw_body, source);
        let rendered = error.to_string();

        assert!(
            rendered
                .starts_with("failed to parse Anthropic response for model Himalaya-opus-4-6: "),
