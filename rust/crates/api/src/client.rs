    ...
    pub fn from_model_with_anthropic_auth(
        ...
        match providers::detect_provider_kind(&resolved_model) {
            ...
            ProviderKind::OpenAi => {
                ...
                Ok(Self::OpenAi(OpenAiCompatClient::from_env_or_base_url(
                    config,
                )?))
            }
        }
...
mod tests {
    ...
    #[test]
    fn openai_base_url_without_api_key_still_builds_client() {
        let _lock = env_lock();
        ...
        let client = ProviderClient::from_model("llama3.2")
        ...
        match client {
            ProviderClient::OpenAi(openai_client) => {
                assert!(
                    ...
                    "expected the local OpenAI-compatible base URL, got: {}",
                    openai_client.base_url()
                );
            }
            other => panic!(
                "expected OpenAi client for local OpenAI-compatible endpoint, got: {other:?}"
            ),
        }
    }
