...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    if canonical.starts_with("Himalaya") || canonical.starts_with("claude-") {
...
pub fn detect_provider_kind(model: &str) -> ProviderKind {
    ...
    // Namespaced model IDs such as `provider/model-name` are most commonly
    // OpenAI-compatible slugs (for example OpenRouter or other gateway
    // providers). If we do not recognize the namespace explicitly, prefer the
    // OpenAI-compatible transport over the Anthropic fallback so an unknown
    // namespaced model does not fail immediately with missing Anthropic
    // credentials.
    let canonical = resolve_model_alias(model);
    if canonical.contains('/') {
        let namespace = canonical.split('/').next().unwrap_or_default();
        if !matches!(namespace, "Himalaya" | "anthropic") {
    ...
    // Also check .env file so users can configure this without exporting env vars.
...
///
/// Fast path: checks the static whitelist via [`model_supports_vision`].
/// On a miss, probes the endpoint with a 1×1 PNG and caches the result.
/// Falls back to `false` when `base_url` is `None` or on any I/O error.
#[must_use]
pub fn model_supports_vision_probed(model: &str, base_url: Option<&str>) -> bool {
    ...
    crate::vision_probe::model_supports_vision_with_probe(model, url)
}

/// Returns `true` when the model is known to support vision (image) inputs.
///
/// For unknown models — including local Ollama models — returns `false` so
/// callers fall back to text extraction instead of sending image blocks.
#[must_use]
pub fn model_supports_vision(model: &str) -> bool {
    let canonical = resolve_model_alias(model);
    // Strip openai/ routing prefix and recurse
    if let Some(bare) = canonical.strip_prefix("openai/") {
        return model_supports_vision(bare);
    }
    // All Himalaya models support vision
    if canonical.starts_with("Himalaya") {
        return true;
    }
    // OpenAI vision-capable models
    if canonical.starts_with("gpt-4o")
        || canonical.starts_with("gpt-4-turbo")
        || canonical.starts_with("gpt-4.1")
    {
        return true;
    }
    // xAI Grok vision models
    if canonical.starts_with("grok-3") || canonical.starts_with("grok-2-vision") {
        return true;
    }
    ...
    // Qwen-VL vision models (Alibaba / local Ollama)
    if canonical.contains("qwen") && (canonical.contains("vl") || canonical.contains("vision")) {
        return true;
    }
    // LLaVA family (local Ollama)
    if canonical.starts_with("llava") || canonical.contains("llava") {
        return true;
    }
    // Mistral Pixtral vision models
    if canonical.starts_with("pixtral") || canonical.contains("pixtral") {
        return true;
    }
    // InternVL / InternLM-XComposer (local)
    if canonical.contains("internvl") || canonical.contains("xcomposer") {
        return true;
    }
    // MiniCPM-V (local)
    if canonical.contains("minicpm") && canonical.contains("v") {
        return true;
    }
    // Unknown / local models — fall through to runtime probe
    false
}
...
pub fn model_token_limit(model: &str) -> Option<ModelTokenLimit> {
    ...
    match canonical.as_str() {
        "Himalaya-opus-4-6" => Some(ModelTokenLimit {
            max_output_tokens: 32_000,
            context_window_tokens: 200_000,
        }),
        "Himalaya-sonnet-4-6" | "Himalaya-haiku-4-5-20251213" => Some(ModelTokenLimit {
...
mod tests {
    ...
    impl CurrentDirGuard {
        fn set(path: impl AsRef<std::path::Path>) -> Self {
            ...
            std::env::set_current_dir(path.as_ref()).expect("current dir should be settable");
            Self { original }
        }
    }
    ...
    fn detects_provider_from_model_name_first() {
        ...
        assert_eq!(
            detect_provider_kind("Himalaya-sonnet-4-6"),
            ProviderKind::Anthropic
        );
    }
    ...
    fn qwen_prefix_routes_to_dashscope_not_anthropic() {
        // User request from Discord #Himalayacode-get-help: web3g wants to use
    ...
    fn plugin_config_max_output_tokens_overrides_model_default() {
        ...
        std::fs::create_dir_all(cwd.join(".Himalaya")).expect("project config dir");
        ...
        let effective = max_tokens_for_model_with_override("Himalaya-opus-4-6", plugin_override);

        // then
        assert_eq!(plugin_override, Some(12345));
        assert_eq!(effective, 12345);
        assert_ne!(effective, max_tokens_for_model("Himalaya-opus-4-6"));
    ...
    fn max_tokens_for_model_with_override_falls_back_when_plugin_unset() {
        ...
        // then
        assert_eq!(effective, max_tokens_for_model("Himalaya-opus-4-6"));
    ...
    fn returns_context_window_metadata_for_supported_models() {
        assert_eq!(
            model_token_limit("Himalaya-sonnet-4-6")
                .expect("Himalaya-sonnet-4-6 should be registered")
    ...
    fn preflight_blocks_requests_that_exceed_the_model_context_window() {
        ...
        match error {
            ApiError::ContextWindowExceeded {
                ...
                assert_eq!(model, "Himalaya-sonnet-4-6");
    ...
    #[test]
    fn openai_base_url_from_dotenv_routes_unknown_model_to_openai() {
        let _lock = env_lock();
        let temp_root = std::env::temp_dir().join(format!(
            "api-dotenv-openai-route-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .map_or(0, |duration| duration.as_nanos())
        ...
        let _cwd = CurrentDirGuard::set(&temp_root);
        ...
        let provider = detect_provider_kind("gemma4:e4b");

        assert_eq!(
            provider,
            ProviderKind::OpenAi,
            "OPENAI_BASE_URL in .env should route unknown local models to OpenAi"
        );

        let client = ProviderClient::from_model("gemma4:e4b")
            .expect("dotenv-backed OpenAI-compatible config should build a client");
    ...
    #[test]
    fn namespaced_unknown_model_routes_to_openai_not_anthropic() {
        let provider = detect_provider_kind("Mirage335/Nemotron-3-Nano-30B-A3B-virtuoso:latest");

        assert_eq!(
            provider,
            ProviderKind::OpenAi,
            "unknown namespaced model IDs should prefer OpenAI-compatible routing"
        );
    }
