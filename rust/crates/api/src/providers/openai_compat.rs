...
impl OpenAiCompatConfig {
    ...
    /// Requested via Discord #Himalayacode-get-help: native Alibaba API for
...
impl OpenAiCompatClient {
    ...
    pub fn from_env_or_base_url(config: OpenAiCompatConfig) -> Result<Self, ApiError> {
        let base_url = read_env_non_empty(config.base_url_env)?;
        let api_key = read_env_non_empty(config.api_key_env)?;
        match (api_key, base_url) {
            (Some(api_key), _) => Ok(Self::new(api_key, config)),
            (None, Some(_)) => Ok(Self::new(String::new(), config)),
    ...
    async fn send_raw_request(
        ...
        self.http
            ...
            .maybe_bearer_auth(&self.api_key)
...
trait RequestBuilderAuthExt {
    fn maybe_bearer_auth(self, api_key: &str) -> reqwest::RequestBuilder;
}
...
impl StreamState {
    ...
    fn ingest_chunk(&mut self, chunk: ChatCompletionChunk) -> Result<Vec<StreamEvent>, ApiError> {
        ...
        for choice in chunk.choices {
            // DeepSeek-R1 and compatible models stream reasoning before content.
            if let Some(rc) = choice.delta.reasoning_content.filter(|v| !v.is_empty()) {
                ...
                events.push(StreamEvent::ContentBlockDelta(ContentBlockDeltaEvent {
                    index: 0,
                    delta: ContentBlockDelta::ThinkingDelta { thinking: rc },
                }));
            }
            ...
            for tool_call in choice.delta.tool_calls {
                // Close the thinking block before opening tool call blocks.
                if self.thinking_started && !self.thinking_finished {
                    self.thinking_finished = true;
                    events.push(StreamEvent::ContentBlockStop(ContentBlockStopEvent {
                        index: 0,
...
struct ChunkDelta {
    #[serde(default)]
    content: Option<String>,
    /// DeepSeek-R1 and compatible models return chain-of-thought here.
    #[serde(default)]
...
fn is_reasoning_model(model: &str) -> bool {
    ...
    canonical.starts_with("o1")
        ...
        || canonical.starts_with("deepseek-r")
...
fn translate_message(message: &InputMessage) -> Vec<Value> {
    match message.role.as_str() {
        ...
        _ => {
            ...
            let flush_user_parts =
                |translated: &mut Vec<Value>, user_parts: &mut Vec<Value>, has_image: &mut bool| {
            ...
            for block in &message.content {
                match block {
                    InputContentBlock::Text { text } => user_parts.push(json!({
                        "type": "text",
                        "text": text,
                    })),
                    InputContentBlock::Image { data, media_type } => {
                        has_image = true;
                    ...
                    InputContentBlock::ToolResult {
                        ...
                        flush_user_parts(&mut translated, &mut user_parts, &mut has_image);
                        translated.push(json!({
                            "role": "tool",
...
mod tests {
    ...
    #[test]
    fn user_text_blocks_stay_in_one_openai_message() {
        let payload = build_chat_completion_request(
            &MessageRequest {
                ...
                messages: vec![InputMessage {
                    ...
                    content: vec![
                        ...
                        InputContentBlock::Text {
                            text: "summarize the paper".to_string(),
                        },
                    ],
                }],
                stream: false,
    ...
    #[test]
    fn openai_compatible_client_allows_custom_base_url_without_api_key() {
        let _lock = env_lock();
        let _base_url = EnvVarGuard::set("OPENAI_BASE_URL", Some("http://127.0.0.1:11434/v1"));
        let _api_key = EnvVarGuard::set("OPENAI_API_KEY", None);

        let client = OpenAiCompatClient::from_env_or_base_url(OpenAiCompatConfig::openai())
            .expect("custom OpenAI-compatible base URL should allow missing api key");

        assert!(
            client.base_url().contains("127.0.0.1:11434"),
            "expected local base URL to be used, got: {}",
            client.base_url()
        );
    }
    ...
    fn grok_3_mini_is_reasoning_model() {
        ...
        assert!(!is_reasoning_model("Himalaya-sonnet-4-6"));
    }
