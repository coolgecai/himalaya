...
async fn send_message_posts_json_and_parses_response() {
    ...
    let body = concat!(
        ...
        "\"type\":\"message\",",
        "\"role\":\"assistant\",",
        "\"content\":[{\"type\":\"text\",\"text\":\"Hello from Himalaya\"}],",
        "\"model\":\"Himalaya-3-7-sonnet-latest\",",
    ...
    assert_eq!(
        ...
        vec![OutputContentBlock::Text {
            text: "Hello from Himalaya".to_string(),
        }]
    );
    ...
    assert_eq!(
        ...
        Some("Himalaya-code/0.1.0")
    );
    assert_eq!(
        request.headers.get("anthropic-beta").map(String::as_str),
        Some("Himalaya-code-20250219,prompt-caching-scope-2026-01-05")
    );
    let body: serde_json::Value =
        serde_json::from_str(&request.body).expect("request body should be json");
    assert_eq!(
        body.get("model").and_then(serde_json::Value::as_str),
        Some("Himalaya-3-7-sonnet-latest")
    );
...
async fn send_message_applies_request_profile_and_records_telemetry() {
    ...
    let client = AnthropicClient::new("test-key")
        ...
        .with_client_identity(
            ClientIdentity::new("Himalaya-code", "9.9.9").with_runtime("rust-cli"),
        )
        .with_beta("tools-2026-04-01")
        .with_extra_body_param("metadata", json!({"source": "Himalayad-code"}))
    ...
    assert_eq!(
        request.headers.get("anthropic-beta").map(String::as_str),
        Some("Himalaya-code-20250219,prompt-caching-scope-2026-01-05,tools-2026-04-01")
    );
    assert_eq!(
        request.headers.get("user-agent").map(String::as_str),
        Some("Himalaya-code/9.9.9")
    );
    let body: serde_json::Value =
        serde_json::from_str(&request.body).expect("request body should be json");
    assert_eq!(body["metadata"]["source"], json!("Himalayad-code"));
    assert!(
        body.get("betas").is_none(),
        "betas must travel via the anthropic-beta header, not the request body"
    );
...
async fn stream_message_parses_sse_events_with_tool_use() {
    ...
    std::env::set_var("Himalaya_CONFIG_HOME", &temp_root);
    let state = Arc::new(Mutex::new(Vec::<CapturedRequest>::new()));
    let sse = concat!(
        "event: message_start\n",
        "data: {\"type\":\"message_start\",\"message\":{\"id\":\"msg_stream\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[],\"model\":\"Himalaya-3-7-sonnet-latest\",\"stop_reason\":null,\"stop_sequence\":null,\"usage\":{\"input_tokens\":8,\"cache_creation_input_tokens\":13,\"cache_read_input_tokens\":21,\"output_tokens\":0}}}\n\n",
    ...
    let server = spawn_server(
        ...
        vec![
            ...
            http_response(
                "200 OK",
                "application/json",
                "{\"id\":\"msg_retry\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"text\",\"text\":\"Recovered\"}],\"model\":\"Himalaya-3-7-sonnet-latest\",\"stop_reason\":\"end_turn\",\"stop_sequence\":null,\"usage\":{\"input_tokens\":3,\"output_tokens\":2}}",
            ),
        ],
    )
    .await;
...
async fn provider_client_dispatches_anthropic_requests() {
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"msg_provider\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"text\",\"text\":\"Dispatched\"}],\"model\":\"Himalaya-3-7-sonnet-latest\",\"stop_reason\":\"end_turn\",\"stop_sequence\":null,\"usage\":{\"input_tokens\":3,\"output_tokens\":2}}",
        )],
    )
    .await;
    ...
    let server = spawn_server(
        ...
        vec![
            ...
            http_response(
                "200 OK",
                "application/json",
                "{\"id\":\"msg_exp_retry\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"text\",\"text\":\"Recovered after 5\"}],\"model\":\"Himalaya-3-7-sonnet-latest\",\"stop_reason\":\"end_turn\",\"stop_sequence\":null,\"usage\":{\"input_tokens\":3,\"output_tokens\":2}}",
            ),
        ],
    )
    .await;
...
async fn send_message_reuses_recent_completion_cache_entries() {
    ...
    let server = spawn_server(
        ...
        vec![http_response(
            "200 OK",
            "application/json",
            "{\"id\":\"msg_cached\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"text\",\"text\":\"Cached once\"}],\"model\":\"Himalaya-3-7-sonnet-latest\",\"stop_reason\":\"end_turn\",\"stop_sequence\":null,\"usage\":{\"input_tokens\":3,\"cache_creation_input_tokens\":5,\"cache_read_input_tokens\":4000,\"output_tokens\":2}}",
        )],
    )
    .await;
    ...
    let server = spawn_server(
        ...
        vec![
            http_response(
                "200 OK",
                "application/json",
                "{\"id\":\"msg_one\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"text\",\"text\":\"One\"}],\"model\":\"Himalaya-3-7-sonnet-latest\",\"stop_reason\":\"end_turn\",\"stop_sequence\":null,\"usage\":{\"input_tokens\":3,\"cache_creation_input_tokens\":5,\"cache_read_input_tokens\":6000,\"output_tokens\":2}}",
            ),
            http_response(
                "200 OK",
                "application/json",
                "{\"id\":\"msg_two\",\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"text\",\"text\":\"Two\"}],\"model\":\"Himalaya-3-7-sonnet-latest\",\"stop_reason\":\"end_turn\",\"stop_sequence\":null,\"usage\":{\"input_tokens\":3,\"cache_creation_input_tokens\":0,\"cache_read_input_tokens\":1000,\"output_tokens\":2}}",
            ),
        ],
    )
    .await;
    ...
    let mut stream = client
        .stream_message(&MessageRequest {
            model: std::env::var("ANTHROPIC_MODEL")
                .unwrap_or_else(|_| "Himalaya-3-7-sonnet-latest".to_string()),
            max_tokens: 32,
...
fn sample_request(stream: bool) -> MessageRequest {
    MessageRequest {
        model: "Himalaya-3-7-sonnet-latest".to_string(),
        max_tokens: 64,
        messages: vec![InputMessage {
            role: "user".to_string(),
            content: vec![
