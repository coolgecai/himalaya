...
const STARTER_Himalaya_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
...
const GITIGNORE_COMMENT: &str = "# Himalaya Code local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".Himalaya/settings.local.json", ".Himalaya/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let Himalaya_dir = cwd.join(".Himalaya");
    artifacts.push(InitArtifact {
        name: ".Himalaya/",
        status: ensure_dir(&Himalaya_dir)?,
    });

    let Himalaya_json = cwd.join(".Himalaya.json");
    artifacts.push(InitArtifact {
        name: ".Himalaya.json",
        status: write_file_if_missing(&Himalaya_json, STARTER_Himalaya_JSON)?,
    });
    ...
    let Himalaya_md = cwd.join("Himalaya.md");
    let content = render_init_Himalaya_md(cwd);
    artifacts.push(InitArtifact {
        name: "Himalaya.md",
        status: write_file_if_missing(&Himalaya_md, &content)?,
    });
...
pub(crate) fn render_init_Himalaya_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# Himalaya.md".to_string(),
        String::new(),
        "This file provides guidance to Himalaya Code (Himalayacode.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.Himalaya.json`; reserve `.Himalaya/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `Himalaya.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_Himalaya_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("rusty-Himalaya-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".Himalaya/"));
        assert!(rendered.contains(".Himalaya.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore"));
        assert!(rendered.contains("Himalaya.md"));
        assert!(rendered.contains("created"));
        assert!(root.join(".Himalaya").is_dir());
        assert!(root.join(".Himalaya.json").is_file());
        assert!(root.join("Himalaya.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".Himalaya.json")).expect("read Himalaya json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"read-only\"\n",
        ...
        assert!(gitignore.contains(".Himalaya/settings.local.json"));
        assert!(gitignore.contains(".Himalaya/sessions/"));
        let Himalaya_md = fs::read_to_string(root.join("Himalaya.md")).expect("read Himalaya md");
        assert!(Himalaya_md.contains("Languages: Rust."));
        assert!(Himalaya_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("Himalaya.md"), "custom guidance\n")
            .expect("write existing Himalaya md");
        fs::write(root.join(".gitignore"), ".Himalaya/settings.local.json\n")
            .expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first.render().contains("Himalaya.md"));
        assert!(first.render().contains("skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".Himalaya/"));
        assert!(second_rendered.contains(".Himalaya.json"));
        assert!(second_rendered.contains("skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore"));
        assert!(second_rendered.contains("Himalaya.md"));
        assert_eq!(
            fs::read_to_string(root.join("Himalaya.md")).expect("read existing Himalaya md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(
            gitignore.matches(".Himalaya/settings.local.json").count(),
            1
        );
        assert_eq!(gitignore.matches(".Himalaya/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_Himalaya_md(Path::new(&root));
