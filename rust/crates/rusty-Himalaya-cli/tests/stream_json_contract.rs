...
fn daemon_report_emits_policy_review_event() {
    ...
    assert!(report["health"]["status"].is_string());
    assert!(report["health"]["next_action"].is_string());
...
fn assert_stream_event_schema(event: &Value) {
        ...
        .expect("event type should be present")
    {
        ...
        "task_status" => {
            assert!(
                event["task"].is_object(),
                "task_status requires task object: {event:?}"
            );
            assert!(
                event.get("plan_progress").is_some(),
                "task_status requires plan_progress: {event:?}"
            );
            assert!(
                event["verification"].is_object(),
                "task_status requires verification object: {event:?}"
            );
            assert!(
                event["current_blocker"].is_object(),
                "task_status requires current_blocker object: {event:?}"
            );
            assert!(
                event["ledger"].is_array(),
                "task_status requires ledger array: {event:?}"
            );
        }
        "task_node_retry" => {
            assert!(
        ...
        "task_scheduler_daemon_report" => {
            ...
            assert!(
                event["health"].is_object(),
                "task_scheduler_daemon_report requires health object: {event:?}"
            );
            assert_non_empty_string(&event["runs_path"]);
            if event.get("runs").is_some() {
                assert!(
        ...
        "task_scheduler_daemon_evaluation" => {
            ...
            if event.get("health").is_some() {
                assert!(
                    event["health"].is_object(),
                    "task_scheduler_daemon_evaluation health must be object when present: {event:?}"
                );
            }
            assert_non_empty_string(&event["runs_path"]);
        }
