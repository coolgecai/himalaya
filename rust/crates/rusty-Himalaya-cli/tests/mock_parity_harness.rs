...
fn clean_env_cli_reaches_mock_anthropic_service_across_scripted_parity_scenarios() {
    ...
    let cases = [
        ...
        ScenarioCase {
            name: "repo_replay_eval",
            permission_mode: "read-only",
            allowed_tools: Some("read_file,grep_search"),
            stdin: None,
            prepare: prepare_repo_replay_fixture,
            assert: assert_repo_replay_eval,
            extra_env: None,
            resume_session: None,
        },
    ];
...
fn run_case(case: ScenarioCase, workspace: &HarnessWorkspace, base_url: &str) -> ScenarioRun {
    ...
    let output = if let Some(stdin) = case.stdin {
        ...
        child.wait_with_output().expect("Himalaya should finish")
    } else {
...
fn prepare_auto_compact_fixture(workspace: &HarnessWorkspace) {
    let sessions_dir = workspace.root.join(".Himalaya").join("sessions");
...
fn prepare_repo_replay_fixture(workspace: &HarnessWorkspace) {
    ...
    .expect("repo replay lib fixture should write");
    fs::write(
        src_dir.join("nested.rs"),
        "pub fn gamma() -> bool { true }\n",
    )
    .expect("repo replay nested fixture should write");
}
...
fn prepare_plugin_fixture(workspace: &HarnessWorkspace) {
    ...
    let manifest_dir = plugin_root.join(".Himalaya-plugin");
...
fn assert_repo_replay_eval(_: &HarnessWorkspace, run: &ScenarioRun) {
    assert_eq!(run.response["iterations"], Value::from(2));
    let tool_uses = run.response["tool_uses"]
        .as_array()
        .expect("tool uses array");
    assert_eq!(
        tool_uses.len(),
        2,
        "repo replay should issue parallel reads"
    );
    assert_eq!(tool_uses[0]["name"], Value::String("read_file".to_string()));
    assert_eq!(
        tool_uses[1]["name"],
    ...
    assert_eq!(
        tool_uses[1]["input"],
        Value::String(r#"{"pattern":"fn [a-z_]","path":"src","output_mode":"count"}"#.to_string())
    );
    let tool_results = run.response["tool_results"]
        .as_array()
        .expect("tool results array");
    assert_eq!(
        tool_results.len(),
        2,
        "repo replay should return both tool results"
    );
    assert!(tool_results
        .iter()
        .all(|result| result["is_error"] == Value::Bool(false)));
    assert!(run.response["message"]
        .as_str()
        .expect("message text")
        .contains("repo replay eval complete"));
    assert!(run.response["message"]
        .as_str()
        .expect("message text")
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "Himalaya-mock-parity-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
