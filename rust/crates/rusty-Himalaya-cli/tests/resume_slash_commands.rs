...
fn resumed_binary_accepts_slash_commands_with_arguments() {
    ...
    assert!(stdout.contains("Resume previous  Himalaya --resume"));
...
fn status_command_applies_cli_flags_end_to_end() {
    ...
    assert!(stdout.contains("Model            Himalaya-sonnet-4-6"));
...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    let config_home = temp_dir.join("home").join(".Himalaya");
    fs::create_dir_all(project_dir.join(".Himalaya")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".Himalaya").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    .expect("local config should write");

    // when
    let output = run_Himalaya_with_env(
        &project_dir,
        &[
            "--resume",
            session_path.to_str().expect("utf8 path"),
...
fn resume_latest_restores_the_most_recent_managed_session() {
    ...
    let project_dir = temp_dir.join("project");
    let sessions_dir = project_dir.join(".Himalaya").join("sessions");
    ...
    // when
    let output = run_Himalaya(&project_dir, &["--resume", "latest", "/status"]);
...
fn resumed_status_surfaces_persisted_model() {
    ...
    let mut session = Session::new();
    session.model = Some("Himalaya-sonnet-4-6".to_string());
    session
    ...
    assert_eq!(parsed["kind"], "status");
    assert_eq!(
        parsed["model"], "Himalaya-sonnet-4-6",
...
#[test]
fn resumed_tasks_list_emits_structured_json() {
    let temp_dir = unique_temp_dir("resume-tasks-json");
    let project_dir = temp_dir.join("project");
    let tasks_dir = project_dir.join(".Himalaya").join("tasks");
    ...
    let task = registry.create("inspect the task registry", Some("fixture task"));
    registry
        .save_to_dir(&tasks_dir)
        .expect("task registry should persist");
    ...
    let stdout = String::from_utf8(output.stdout).expect("utf8");
    let parsed: Value = serde_json::from_str(stdout.trim()).expect("should be json");
    assert_eq!(parsed["type"], "task_list");
    assert_eq!(parsed["tasks"].as_array().expect("tasks array").len(), 1);
    assert_eq!(parsed["tasks"][0]["task_id"], task.task_id);
}

#[test]
fn resumed_cron_list_emits_structured_json() {
    let temp_dir = unique_temp_dir("resume-cron-json");
    let project_dir = temp_dir.join("project");
    let cron_dir = project_dir.join(".Himalaya").join("cron");
    fs::create_dir_all(&cron_dir).expect("cron dir should exist");
    ...
    let entry = registry.create("7 * * * *", "check the deploy", Some("hourly check"));
    registry
        .save_to_dir(&cron_dir)
        .expect("cron registry should persist");
    ...
    let stdout = String::from_utf8(output.stdout).expect("utf8");
    let parsed: Value = serde_json::from_str(stdout.trim()).expect("should be json");
    assert_eq!(parsed["type"], "cron_list");
    assert_eq!(parsed["crons"].as_array().expect("crons array").len(), 1);
    assert_eq!(parsed["crons"][0]["cron_id"], entry.cron_id);
}

#[test]
fn resumed_plan_emits_structured_json() {
    let temp_dir = unique_temp_dir("resume-plan-json");
    fs::create_dir_all(&temp_dir).expect("temp dir should exist");
    let session_path = temp_dir.join("session.jsonl");
    Session::new()
        .save_to_path(&session_path)
...
fn run_Himalaya(current_dir: &Path, args: &[&str]) -> Output {
    run_Himalaya_with_env(current_dir, args, &[])
}

fn run_Himalaya_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
