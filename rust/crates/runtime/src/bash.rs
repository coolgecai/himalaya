...
fn prepare_command(
    ...
    #[cfg(windows)]
    let mut prepared = {
        // Switch to UTF-8 code page before running the command so that
        // system error messages and program output are encoded as UTF-8
        // rather than the default system code page (e.g. CP936 on Chinese Windows).
        let utf8_command = format!("chcp 65001 >nul 2>&1 & {command}");
        let mut cmd = Command::new("cmd");
        cmd.arg("/C").arg(utf8_command).current_dir(cwd);
        cmd
    };
