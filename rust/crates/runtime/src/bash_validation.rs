...
const INLINE_CODE_INTERPRETERS: &[&str] = &["python", "python3", "node", "ruby", "perl", "php"];
...
pub fn validate_read_only(command: &str, mode: PermissionMode) -> ValidationResult {
    ...
    if first_command != "tee" && contains_bare_command(command, "tee") {
        return ValidationResult::Block {
            reason: "Command uses 'tee' to write output and is not allowed in read-only mode"
                .to_string(),
        };
    }
    ...
    if contains_inline_code_execution(command) {
        return ValidationResult::Block {
            reason: "inline interpreter execution is not allowed in read-only mode".to_string(),
        };
    }
...
pub fn validate_sed(command: &str, mode: PermissionMode) -> ValidationResult {
    let first = extract_first_command(command);
    if first != "sed" && !contains_bare_command(command, "sed") {
        return ValidationResult::Allow;
    }

    if mode == PermissionMode::ReadOnly && contains_sed_in_place(command) {
        return ValidationResult::Block {
            reason: "sed -i / --in-place editing is not allowed in read-only mode".to_string(),
        };
    }
...
mod tests {
    ...
    #[test]
    fn blocks_read_only_shell_write_escape_patterns() {
        let cases = [
        ...
        for command in cases {
            assert!(
                ...
                "expected read-only mode to block {command}"
            );
        }
    }
