...
pub fn read_file(
    ...
    // Detect binary files — but first try document extraction for known formats
    if is_binary_file(&absolute_path)? {
        use file_extract::{extract_file, FileContent};
        match extract_file(&absolute_path, false) {
            ...
            Ok(FileContent::Image { media_type, .. }) => {
                ...
                let placeholder = format!(
                    "[Image file: {name} ({mime}) — use @{name} syntax to attach as image for vision-capable models]"
                );
                return Ok(ReadFileOutput {
                    kind: String::from("text"),
                    file: TextFilePayload {
            ...
            Err(file_extract::ExtractError::UnsupportedType(ext)) => {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("unsupported binary file type: .{ext}"),
                ));
            }
            Err(_) => {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    "file appears to be binary and could not be extracted",
                ));
            }
        }
    }
...
/// Output envelope for the `generate_file` tool.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct GenerateFileOutput {
    #[serde(rename = "filePath")]
    pub file_path: String,
    pub format: String,
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> std::path::PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("Himalayad-native-{name}-{unique}"))
    }
    ...
    #[test]
    fn workspace_write_and_edit_enforce_canonical_boundaries() {
        let base = temp_path("workspace-canonical-boundary");
        let workspace = base.join("workspace");
        let child = workspace.join("child");
        std::fs::create_dir_all(&child).expect("workspace child should be created");

        let inside_missing = child.join("../created.txt");
        write_file_in_workspace(
            inside_missing.to_string_lossy().as_ref(),
            "safe",
            &workspace,
        )
        .expect("missing file under canonical workspace parent should write");

        edit_file_in_workspace(
            workspace.join("created.txt").to_string_lossy().as_ref(),
            "safe",
            "safer",
            false,
            &workspace,
        )
        .expect("edit inside workspace should succeed");

        let outside = base.join("outside.txt");
        let result =
            write_file_in_workspace(outside.to_string_lossy().as_ref(), "unsafe", &workspace);
        assert!(result.is_err());
        assert_eq!(
    ...
    #[test]
    fn workspace_read_rejects_symlink_escape_target() {
        let base = temp_path("workspace-symlink-read-boundary");
        let workspace = base.join("workspace");
        std::fs::create_dir_all(&workspace).expect("workspace dir should be created");
        let outside = base.join("outside.txt");
        ...
        #[cfg(unix)]
        {
            ...
            let result =
                read_file_in_workspace(link.to_string_lossy().as_ref(), None, None, &workspace);
            assert!(result.is_err());
            assert_eq!(
                result.unwrap_err().kind(),
                std::io::ErrorKind::PermissionDenied
