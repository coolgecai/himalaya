...
pub fn compact_session_with(
    ...
    let keep_from = {
        ...
        // Ensure thinking blocks are preserved. If the message just before
        // the boundary is an assistant message with thinking blocks, include
        // it — the API requires thinking blocks to be passed back verbatim.
        if k > compacted_prefix_len {
            let preceding = &session.messages[k - 1];
            if preceding.role == MessageRole::Assistant {
                let has_thinking = preceding.blocks.iter().any(|b| {
                    matches!(
                        b,
...
fn summarize_messages(messages: &[ConversationMessage]) -> String {
    ...
    if let Some(objective) = active_objective {
        lines.push(format!("- Active objective: {objective}"));
    }
    ...
    if !evidence_sources.is_empty() {
        lines.push("- Evidence collected before compaction:".to_string());
        lines.extend(
            evidence_sources
                .into_iter()
    ...
    if !successful_tools.is_empty() {
        lines.push("- Successful tool observations:".to_string());
        lines.extend(
            successful_tools
                .into_iter()
                .map(|item| format!("  - {item}")),
    ...
    if !failed_tools.is_empty() {
        lines.push("- Failed or blocked tool attempts:".to_string());
        lines.extend(failed_tools.into_iter().map(|item| format!("  - {item}")));
    }
    ...
    if !pending_work.is_empty() {
        lines.push("- Pending work / next actions:".to_string());
        lines.extend(pending_work.into_iter().map(|item| format!("  - {item}")));
    }

    if let Some(current_work) = current_work {
        lines.push(format!(
            "- Current work at compaction boundary: {current_work}"
        ));
    }

    lines.push("- Resume guidance: continue the active objective from this state; prefer current file/tool evidence over guesses, and rerun searches if referenced files may have changed.".to_string());
...
fn infer_active_objective(messages: &[ConversationMessage]) -> Option<String> {
    messages
        .iter()
        .rev()
        .filter(|message| message.role == MessageRole::User)
        ...
        .find(|text| {
            ...
            !lowered.starts_with("workspace analysis requires local evidence")
                && !lowered.starts_with("tool result")
                && !text.trim().is_empty()
        })
        .map(|text| truncate_summary(text, 220))
}

fn collect_tool_result_summaries(messages: &[ConversationMessage]) -> (Vec<String>, Vec<String>) {
    ...
    for block in messages.iter().flat_map(|message| message.blocks.iter()) {
        ...
        let item = format!("{tool_name}: {}", truncate_summary(output, 180));
        if *is_error {
            failed.push(item);
        } else {
...
fn collect_evidence_sources(messages: &[ConversationMessage]) -> Vec<String> {
    let mut sources = messages
        .iter()
        .flat_map(|message| message.blocks.iter())
        .filter_map(|block| match block {
            ContentBlock::ToolUse { name, input, .. } => Some(format!("{name}: {input}")),
            ContentBlock::ToolResult { output, .. } => {
                let files = extract_file_candidates(output);
                (!files.is_empty()).then(|| {
                    format!(
                        "files: {}",
                        files.into_iter().take(5).collect::<Vec<_>>().join(", ")
                    )
                })
            }
            ContentBlock::Text { .. }
        ...
        .map(|item| truncate_summary(&item, 180))
        .collect::<Vec<_>>();
    sources.sort();
    sources.dedup();
...
mod tests {
    ...
    fn extracts_key_files_from_message_content() {
        let files = collect_key_files(&[ConversationMessage::user_text(
            "Update rust/crates/runtime/src/compact.rs and rust/crates/rusty-Himalaya-cli/src/main.rs next.",
        )]);
