...
impl CronRegistry {
    ...
    pub fn save_to_dir(&self, dir: &Path) -> io::Result<()> {
        ...
        let snapshot = CronRegistrySnapshot {
        ...
        fs::write(dir.join("crons.json"), format!("{json}\n"))
    }

    pub fn load_from_dir(dir: &Path) -> io::Result<Self> {
        let contents = fs::read_to_string(dir.join("crons.json"))?;
        let snapshot = serde_json::from_str::<CronRegistrySnapshot>(&contents)
            .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))?;
        let entries = snapshot
            .entries
            .into_iter()
            .map(|entry| (entry.cron_id.clone(), entry))
            .collect();
        Ok(Self {
...
mod tests {
    ...
    #[test]
    fn cron_registry_persists_and_loads_snapshot() {
        let dir = std::env::temp_dir().join(format!(
            "Himalaya-cron-registry-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
        ...
        let registry = CronRegistry::new();
        let entry = registry.create("17 * * * *", "Check deploy", Some("hourly"));
        registry.record_run(&entry.cron_id).expect("record run");
        registry.save_to_dir(&dir).expect("save snapshot");

        let loaded = CronRegistry::load_from_dir(&dir).expect("load snapshot");
        let fetched = loaded.get(&entry.cron_id).expect("entry should load");
        assert_eq!(fetched.schedule, "17 * * * *");
        assert_eq!(fetched.prompt, "Check deploy");
        assert_eq!(fetched.description.as_deref(), Some("hourly"));
