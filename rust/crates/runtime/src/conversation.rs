    ...
    T: ToolExecutor,
{
    ...
    fn reflect_on_outcome(
        ...
        // Error pattern recognition: collect tool failures.
        let mut total_tools = 0usize;
        let mut failed_tools = 0usize;
        let mut examples = Vec::new();
        for msg in &summary.tool_results {
            total_tools += 1;
            if let Some(ContentBlock::ToolResult { tool_name, output, is_error, .. }) = msg.blocks.first() {
                if *is_error {
                    failed_tools += 1;
                    if examples.len() < 3 {
                        examples.push(format!("{}: {}", tool_name, output.chars().take(256).collect::<String>()));
                    }
                }
            }
        }

        if total_tools > 0 && failed_tools * 2 >= total_tools {
            let note = format!(
                "Detected repeated tool failures: {}/{} failed. Examples: {}",
                failed_tools,
                total_tools,
                examples.join(" | ")
            );
        ...
        // If we have a chain-of-thought, score confidence and persist low-confidence findings.
            ...
            if cot.confidence < 0.4 {
                let note = format!(
                    ...
                    cot.steps
                        .iter()
                        .map(|s| match s {
                            ReasoningStep::Analysis { content, .. } => format!("A:{}", content.chars().take(80).collect::<String>()),
                            ReasoningStep::Planning { plan, .. } => format!("P:{}", plan.chars().take(80).collect::<String>()),
                            ReasoningStep::Reflection { critique, .. } => format!("R:{}", critique.chars().take(80).collect::<String>()),
                            ReasoningStep::Decision { choice, .. } => format!("D:{}", choice.chars().take(80).collect::<String>()),
                        })
                        .collect::<Vec<_>>()
                        .join(" | ")
                );
        ...
        // Optionally, persist a short turn summary fact for cross-session lookup.
