...
const DECISIONING_FIELDS: &[FieldSpec] = &[
    FieldSpec {
        name: "enabled",
        expected: FieldType::Bool,
    },
...
const DECISIONING_SAFETY_POLICY_FIELDS: &[FieldSpec] = &[
    FieldSpec {
        name: "baseScorePercent",
        expected: FieldType::Number,
    },
    FieldSpec {
    ...
    FieldSpec {
        name: "protectedTermPenaltyPercent",
        expected: FieldType::Number,
    },
    FieldSpec {
        name: "constraintOverlapPenaltyPercent",
        expected: FieldType::Number,
    },
    FieldSpec {
        name: "parallelStepPenaltyPercent",
        expected: FieldType::Number,
    },
    FieldSpec {
    ...
    FieldSpec {
        name: "highComplexityPenaltyPercent",
        expected: FieldType::Number,
    },
    FieldSpec {
        name: "lowConfidenceThresholdPercent",
        expected: FieldType::Number,
    },
    FieldSpec {
...
pub fn validate_config_file(
    ...
    if let Some(decisioning) = object.get("decisioning").and_then(JsonValue::as_object) {
        result.merge(validate_object_keys(
            decisioning,
            DECISIONING_FIELDS,
            "decisioning",
        ...
        if let Some(safety_policy) = decisioning
            .get("safetyPolicy")
            .and_then(JsonValue::as_object)
        {
            result.merge(validate_object_keys(
                safety_policy,
                DECISIONING_SAFETY_POLICY_FIELDS,
                "decisioning.safetyPolicy",
                source,
                &path_display,
            ));
        }
...
mod tests {
  ...
  "sandbox": {"enabled": false}
    ...
    fn check_unsupported_format_rejects_toml() {
        // given
        let path = PathBuf::from("/home/.Himalaya/settings.toml");
    ...
    fn check_unsupported_format_allows_json() {
        // given
        let path = PathBuf::from("/home/.Himalaya/settings.json");
