...
/// Configurable safety policy for the decisioning engine.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct DecisioningSafetyPolicyConfig {
    base_score_percent: u8,
    review_threshold_percent: u8,
    deny_threshold_percent: u8,
    protected_terms: Vec<String>,
    protected_term_penalty_percent: u8,
    constraint_overlap_penalty_percent: u8,
    parallel_step_penalty_percent: u8,
    high_complexity_threshold: u8,
    high_complexity_penalty_percent: u8,
    low_confidence_threshold_percent: u8,
    low_confidence_penalty_percent: u8,
}

impl Default for DecisioningSafetyPolicyConfig {
    fn default() -> Self {
        Self {
            base_score_percent: 5,
            review_threshold_percent: 35,
            deny_threshold_percent: 70,
            protected_terms: vec![
                "delete".to_string(),
                "remove".to_string(),
                "overwrite".to_string(),
            ...
            protected_term_penalty_percent: 18,
            constraint_overlap_penalty_percent: 10,
            parallel_step_penalty_percent: 4,
            high_complexity_threshold: 4,
            high_complexity_penalty_percent: 8,
            low_confidence_threshold_percent: 40,
            low_confidence_penalty_percent: 8,
        }
    }
}
...
/// OAuth client configuration used by the main Himalaya runtime.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".Himalaya.json"),
            |parent| parent.join(".Himalaya.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".Himalaya.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".Himalaya").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".Himalaya").join("settings.local.json"),
            },
        ]
    }
...
impl RuntimeFeatureConfig {
    ...
    #[must_use]
    pub fn with_decisioning(mut self, decisioning: DecisioningConfig) -> Self {
...
impl DecisioningConfig {
    ...
    pub fn with_safety_thresholds(mut self, review_percent: u8, deny_percent: u8) -> Self {
        self.safety_policy = self
            .safety_policy
            .with_review_threshold_percent(review_percent)
            .with_deny_threshold_percent(deny_percent.max(review_percent));
        self
    }

    #[must_use]
    pub fn with_safety_policy(mut self, safety_policy: DecisioningSafetyPolicyConfig) -> Self {
    ...
    #[must_use]
    pub fn safety_review_threshold_percent(&self) -> u8 {
        self.safety_policy.review_threshold_percent()
    }

    #[must_use]
    pub fn safety_deny_threshold_percent(&self) -> u8 {
        self.safety_policy
            .deny_threshold_percent()
            .max(self.safety_policy.review_threshold_percent())
    }

    #[must_use]
    pub fn safety_policy(&self) -> &DecisioningSafetyPolicyConfig {
        &self.safety_policy
    }
}

impl DecisioningSafetyPolicyConfig {
    #[must_use]
    pub fn with_base_score_percent(mut self, percent: u8) -> Self {
        self.base_score_percent = percent.min(100);
        self
    }

    #[must_use]
    pub fn with_review_threshold_percent(mut self, percent: u8) -> Self {
        self.review_threshold_percent = percent.min(100);
        self
    }

    #[must_use]
    pub fn with_deny_threshold_percent(mut self, percent: u8) -> Self {
        self.deny_threshold_percent = percent.min(100);
        self
    }

    #[must_use]
    pub fn with_protected_terms(mut self, protected_terms: Vec<String>) -> Self {
        self.protected_terms = protected_terms;
        self
    }

    #[must_use]
    pub fn with_protected_term_penalty_percent(mut self, percent: u8) -> Self {
        self.protected_term_penalty_percent = percent.min(100);
        self
    }

    #[must_use]
    pub fn with_constraint_overlap_penalty_percent(mut self, percent: u8) -> Self {
        self.constraint_overlap_penalty_percent = percent.min(100);
        self
    }

    #[must_use]
    pub fn with_parallel_step_penalty_percent(mut self, percent: u8) -> Self {
        self.parallel_step_penalty_percent = percent.min(100);
        self
    }

    #[must_use]
    pub fn with_high_complexity_threshold(mut self, threshold: u8) -> Self {
        self.high_complexity_threshold = threshold.max(1);
        self
    }

    #[must_use]
    pub fn with_high_complexity_penalty_percent(mut self, percent: u8) -> Self {
        self.high_complexity_penalty_percent = percent.min(100);
        self
    }

    #[must_use]
    pub fn with_low_confidence_threshold_percent(mut self, percent: u8) -> Self {
        self.low_confidence_threshold_percent = percent.min(100);
        self
    }

    #[must_use]
    pub fn with_low_confidence_penalty_percent(mut self, percent: u8) -> Self {
        self.low_confidence_penalty_percent = percent.min(100);
        self
    }

    #[must_use]
    pub fn base_score_percent(&self) -> u8 {
        self.base_score_percent.min(100)
    }

    #[must_use]
    pub fn review_threshold_percent(&self) -> u8 {
        self.review_threshold_percent.min(100)
    }

    #[must_use]
    pub fn deny_threshold_percent(&self) -> u8 {
        self.deny_threshold_percent.min(100)
    }
    ...
    #[must_use]
    pub fn protected_term_penalty_percent(&self) -> u8 {
        self.protected_term_penalty_percent.min(100)
    }

    #[must_use]
    pub fn constraint_overlap_penalty_percent(&self) -> u8 {
        self.constraint_overlap_penalty_percent.min(100)
    }

    #[must_use]
    pub fn parallel_step_penalty_percent(&self) -> u8 {
        self.parallel_step_penalty_percent.min(100)
    }

    #[must_use]
    pub fn high_complexity_threshold(&self) -> u8 {
        self.high_complexity_threshold.max(1)
    }

    #[must_use]
    pub fn high_complexity_penalty_percent(&self) -> u8 {
        self.high_complexity_penalty_percent.min(100)
    }

    #[must_use]
    pub fn low_confidence_threshold_percent(&self) -> u8 {
        self.low_confidence_threshold_percent.min(100)
    }

    #[must_use]
    pub fn low_confidence_penalty_percent(&self) -> u8 {
        self.low_confidence_penalty_percent.min(100)
    }
}
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("Himalaya_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".Himalaya")))
        .or_else(|| {
            std::env::var_os("USERPROFILE").map(|home| PathBuf::from(home).join(".Himalaya"))
        })
        .unwrap_or_else(|| PathBuf::from(".Himalaya"))
}
...
fn read_optional_json_object(path: &Path) -> Result<Option<ParsedConfigFile>, ConfigError> {
    let is_legacy_config =
        path.file_name().and_then(|name| name.to_str()) == Some(".Himalaya.json");
...
fn parse_optional_decisioning_config(root: &JsonValue) -> Result<DecisioningConfig, ConfigError> {
    let Some(object) = root.as_object() else {
        return Ok(DecisioningConfig::default());
    };
    let Some(value) = object.get("decisioning") else {
        return Ok(DecisioningConfig::default());
    };
    ...
    if let Some(emit_events) =
        optional_bool(decisioning, "emitEvents", "merged settings.decisioning")?
    {
        config = config.with_emit_events(emit_events);
    }
    if let Some(max_parallelism) =
        optional_u32(decisioning, "maxParallelism", "merged settings.decisioning")?
    {
    ...
    if let Some(policy_value) = decisioning.get("safetyPolicy") {
        ...
        config = config.with_safety_policy(parse_decisioning_safety_policy_config(
            policy,
            "merged settings.decisioning.safetyPolicy",
        )?);
    }
...
fn parse_decisioning_safety_policy_config(
    object: &BTreeMap<String, JsonValue>,
    context: &str,
) -> Result<DecisioningSafetyPolicyConfig, ConfigError> {
    let mut policy = DecisioningSafetyPolicyConfig::default();

    if let Some(base_score_percent) = optional_u16(object, "baseScorePercent", context)? {
        policy = policy.with_base_score_percent(base_score_percent.min(100) as u8);
    }
    if let Some(review_threshold_percent) = optional_u16(object, "reviewThresholdPercent", context)?
    {
        policy = policy.with_review_threshold_percent(review_threshold_percent.min(100) as u8);
    }
    if let Some(deny_threshold_percent) = optional_u16(object, "denyThresholdPercent", context)? {
        policy = policy.with_deny_threshold_percent(deny_threshold_percent.min(100) as u8);
    }
    if let Some(protected_terms) = optional_string_array(object, "protectedTerms", context)? {
        policy = policy.with_protected_terms(protected_terms);
    }
    if let Some(protected_term_penalty_percent) =
        optional_u16(object, "protectedTermPenaltyPercent", context)?
    {
        policy = policy
            .with_protected_term_penalty_percent(protected_term_penalty_percent.min(100) as u8);
    }
    if let Some(constraint_overlap_penalty_percent) =
        optional_u16(object, "constraintOverlapPenaltyPercent", context)?
    {
        policy = policy.with_constraint_overlap_penalty_percent(
            constraint_overlap_penalty_percent.min(100) as u8,
        );
    }
    if let Some(parallel_step_penalty_percent) =
        optional_u16(object, "parallelStepPenaltyPercent", context)?
    {
        policy =
            policy.with_parallel_step_penalty_percent(parallel_step_penalty_percent.min(100) as u8);
    }
    if let Some(high_complexity_threshold) =
        optional_u16(object, "highComplexityThreshold", context)?
    {
        policy = policy.with_high_complexity_threshold(high_complexity_threshold.min(255) as u8);
    }
    if let Some(high_complexity_penalty_percent) =
        optional_u16(object, "highComplexityPenaltyPercent", context)?
    {
        policy = policy
            .with_high_complexity_penalty_percent(high_complexity_penalty_percent.min(100) as u8);
    }
    if let Some(low_confidence_threshold_percent) =
        optional_u16(object, "lowConfidenceThresholdPercent", context)?
    {
        policy = policy
            .with_low_confidence_threshold_percent(low_confidence_threshold_percent.min(100) as u8);
    }
    if let Some(low_confidence_penalty_percent) =
        optional_u16(object, "lowConfidencePenaltyPercent", context)?
    {
        policy = policy
            .with_low_confidence_penalty_percent(low_confidence_penalty_percent.min(100) as u8);
    }
...
fn parse_mcp_server_config(
    ...
    match server_type {
        ...
        "Himalayaai-proxy" => Ok(McpServerConfig::ManagedProxy(McpManagedProxyServerConfig {
            url: expect_string(object, "url", context)?.to_string(),
...
mod tests {
    use super::{
        ...
        Himalaya_SETTINGS_SCHEMA_NAME, McpServerConfig, McpTransport, ResolvedPermissionMode,
        RuntimeHookConfig, RuntimePluginConfig,
    };
    use crate::json::JsonValue;
    ...
    #[test]
    fn loads_and_merges_Himalaya_code_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".Himalaya");
        ...
        fs::write(
            home.parent().expect("home parent").join(".Himalaya.json"),
        ...
        fs::write(
            cwd.join(".Himalaya").join("settings.local.json"),
        ...
        assert_eq!(Himalaya_SETTINGS_SCHEMA_NAME, "SettingsSchema");
    ...
    fn parses_provider_fallbacks_chain_with_primary_and_ordered_fallbacks() {
        ...
        assert_eq!(chain.primary(), Some("Himalaya-opus-4-6"));
        assert_eq!(
            chain.fallbacks(),
    ...
    fn parses_user_defined_model_aliases_from_settings() {
        ...
        fs::write(
            home.join("settings.json"),
            r#"{"aliases":{"fast":"Himalaya-haiku-4-5-20251213","smart":"Himalaya-opus-4-6"}}"#,
        )
        .expect("write user settings");
        fs::write(
            cwd.join(".Himalaya").join("settings.local.json"),
            r#"{"aliases":{"smart":"Himalaya-sonnet-4-6","cheap":"grok-3-mini"}}"#,
        )
        .expect("write local settings");
        ...
        assert_eq!(
            ...
            Some("Himalaya-haiku-4-5-20251213")
        );
        assert_eq!(
            aliases.get("smart").map(String::as_str),
            Some("Himalaya-sonnet-4-6")
        );
        assert_eq!(
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".Himalaya");
        let project_settings = cwd.join(".Himalaya").join("settings.json");
        fs::create_dir_all(cwd.join(".Himalaya")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
