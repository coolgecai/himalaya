...
This milestone adds a deterministic Anthropic-compatible mock service plus a reproducible CLI harness for the Rust `Himalaya` binary.

## Artifacts

- `crates/mock-anthropic-service/` — mock `/v1/messages` service
- `crates/rusty-Himalaya-cli/tests/mock_parity_harness.rs` — end-to-end clean-environment harness
