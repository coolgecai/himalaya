...
### Limited Implementations (real behavior, not complete UX)

| Tool | Status | Notes |
|------|--------|-------|
| **AskUserQuestion** | partial | CLI stdin/stdout prompting is implemented; VS Code/webview interaction remains open |
| **McpAuth** | partial | reports registered MCP lifecycle/auth state; full auth flow UX remains open |
| **RemoteTrigger** | partial | bounded basic HTTP request execution is implemented; policy/auth/retry hardening remains open |
| **TestingPermission** | test-only | test-only, low priority |

## Slash Commands: broad parser surface, filtered help/completions

- Slash command specs include upstream-compatible entries; unsupported entries are filtered from REPL completions/help and return a clear "not yet implemented" error in resume mode.
- `/tasks` is now wired to the durable task registry surface for direct CLI, direct slash CLI, REPL, and resumed `list`/`show` inspection paths.
- `/cron list` is now wired to persisted cron registry snapshots for direct CLI, direct slash CLI, REPL, and resumed inspection paths; create/remove update `.Himalaya/cron/crons.json` locally.
- Remaining slash-command work should continue moving entries out of `STUB_COMMANDS` only when they have a real local handler or a safe resume/REPL execution path.
