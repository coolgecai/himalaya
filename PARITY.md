...
- [x] Reproducible clean-environment CLI harness (`rust/crates/rusty-Himalaya-cli/tests/mock_parity_harness.rs`)
...
- [x] Scripted plugin coverage: `plugin_tool_roundtrip`
- [x] Scripted repo replay coverage: `repo_replay_eval`
- [x] Scripted auto-compaction coverage: `auto_compact_triggered`
- [x] Scripted token/cost reporting coverage: `token_cost_reporting`
...
- Token/cost reporting evidence
- Repo replay eval foundation
- Mock parity harness scenario reporting
- PARITY reference coverage checked by `rust/scripts/run_mock_parity_diff.py`
...
- Dedicated validation coverage exists in `rust/crates/runtime/src/bash_validation.rs`.
- Harness coverage proves bash execution and prompt escalation flows; validation-specific coverage should stay green alongside permission-enforcement tests.
- Rust keeps validation, permission enforcement, and subprocess execution as separate runtime modules.
...
- **Why it matters:** `.github/workflows/rust-ci.yml` runs `cargo fmt --all --check` and `cargo test -p rusty-Himalaya-cli`; this lane removed a CI-specific sandbox assumption from runtime behavior.
...
- **Evidence:** `rust/crates/runtime/src/team_cron_registry.rs` adds thread-safe `TeamRegistry` and `CronRegistry`; `rust/crates/tools/src/lib.rs` wires `TeamCreate`, `TeamDelete`, `CronCreate`, `CronDelete`, and `CronList` into those registries.
- **Current state:** team/cron tools now have registry lifecycle behavior on `main`; this branch also persists cron snapshots under `.Himalaya/cron/crons.json` and wires `/cron list` for direct CLI, REPL, and resumed inspection paths. Cron still stops short of a real background scheduler or worker fleet.
...
- `AskUserQuestion` now performs simple stdin/stdout CLI prompting, but it still needs first-class VS Code/webview interactive wiring.
- `RemoteTrigger` now issues bounded HTTP requests for basic methods, but it needs auth policy, allowlist, retry, and event-contract hardening before it should be treated as a production orchestration surface.
- `McpAuth` reports registered MCP server auth/lifecycle state; full auth UX beyond the lifecycle bridge remains limited.
- Task, team, MCP, and LSP are no longer just fixed-payload stubs in `execute_tool()`, but several remain registry-backed approximations rather than full external-runtime integrations.
- Cron now has registry persistence and local `/cron list` inspection coverage; it still lacks an actual scheduler loop.
- Bash deep validation is present in `rust/crates/runtime/src/bash_validation.rs`; remaining bash work is parity-depth and integration coverage, not landing the module itself.
...
- [x] Config merge precedence (user > project > local) — `ConfigLoader::discover()` loads user → project → local, and `loads_and_merges_Himalaya_code_config_files_by_precedence()` verifies the merge order.
...
- [x] All 9 requested lanes are represented in the current Rust workspace, including `bash_validation`.
